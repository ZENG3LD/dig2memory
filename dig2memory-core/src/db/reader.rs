...
pub fn list_crate_nodes(
    ...
    match workspace_id {
        Some(ws) => {
            let mut stmt = conn.prepare(
                "SELECT workspace_id, name, version, manifest_path FROM crate_nodes WHERE workspace_id = ?1",
            )?;
            let result = stmt
        ...
        None => {
            let mut stmt = conn.prepare(
                "SELECT workspace_id, name, version, manifest_path FROM crate_nodes",
            )?;
            let result = stmt
                .query_map([], |row| {
                    Ok(CrateNode {
...
pub fn list_crate_deps(
    ...
    match workspace_id {
        Some(ws) => {
            let mut stmt = conn.prepare(
                "SELECT workspace_id, from_crate, to_crate, is_path_dep FROM crate_dep_edges WHERE workspace_id = ?1",
            )?;
            let result = stmt
                .query_map(params![ws], |row| {
        ...
        None => {
            let mut stmt = conn.prepare(
                "SELECT workspace_id, from_crate, to_crate, is_path_dep FROM crate_dep_edges",
            )?;
            let result = stmt
                .query_map([], |row| {
                    let is_path: i32 = row.get(3)?;
